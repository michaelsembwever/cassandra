...
public class Scrubber implements Closeable
{
    ...
    public Scrubber(ColumnFamilyStore cfs, SSTableReader sstable, boolean skipCorrupted, boolean checkData, OutputHandler outputHandler, boolean isOffline) throws IOException
    {
        ...
        hasIndexFile = sstable.componentsFor(sstable.descriptor).contains(Component.PRIMARY_INDEX);
    ...
    public void scrub()
    {
        ...
        try
        {
            nextIndexKey = hasIndexFile ? ByteBufferUtil.readWithShortLength(indexFile) : null;
            if (hasIndexFile)
            {
            ...
            while (!dataFile.isEOF())
            {
                ...
                assert currentIndexKey != null || !hasIndexFile || indexFile.isEOF();
                ...
                try
                {
                    ...
                    if (hasIndexFile && dataStart != dataStartFromIndex)
                    ...
                    if (hasIndexFile && dataSize != dataSizeFromIndex)
    ...
    private void updateIndexKey()
    {
        ...
        try
        {
            nextIndexKey = !hasIndexFile || indexFile.isEOF() ? null : ByteBufferUtil.readWithShortLength(indexFile);

            nextRowPositionFromIndex = !hasIndexFile || indexFile.isEOF()
                    ? dataFile.length()
